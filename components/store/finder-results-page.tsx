...
export default function FinderResultsPage({
  ...
  products: Product[];
  selection: VehicleFinderSelection;
}) {
  ...
  const productsCountByBrand = useMemo(() => {
    ...
    bikeBrands.forEach(b => {
      counts[b.slug] = products.filter(p => p.bikeBrand === b.slug).length;
    });
    return counts;
  }, [products]);

  const makeMeta = useMemo(() => 
    selection.make && selection.make in vehicleData
      ? vehicleData[selection.make as keyof typeof vehicleData]
      : null
  , [selection]);
  ...
  const selectionLabel = [makeMeta?.label, selection.model, selection.year, selection.engine]
    .filter(Boolean)
    .join(" / ");
  ...
  }, [products, search, selectedCategory, selectedSupplierBrand]);
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-700">
        ...
        <section className="bg-slate-950 py-18">
          <div className="mx-auto max-w-[1400px] px-4 py-16 text-white lg:px-6">
            ...
            <h1 className="font-heading text-4xl font-extrabold sm:text-5xl">
              {selectionLabel || "Matched Motorbike Parts"}
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-slate-300 sm:text-lg sm:leading-8">
              {selectionLabel
                ? "These products match the bike details you selected in the motor parts finder."
                : "Choose a bike from the home-page finder to see matched products here."}
            </p>
            {selectionLabel ? (
              <div className="mt-6 flex flex-wrap gap-3">
                {[makeMeta?.label, selection.model, selection.year, selection.engine]
                  .filter(Boolean)
                  .map((item) => (
                    <span
                      key={item}
          ...
          <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
            <div className="mb-8 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
              <p className="text-sm font-medium text-slate-500">
                Showing {filtered.length} matched products
              </p>
              <Link href="/#finder" className="text-sm font-semibold text-[#dc2626]">
                New finder search
              </Link>
            </div>
            ...
            <div className="grid gap-7 md:grid-cols-2 xl:grid-cols-4">
              {filtered.length === 0 ? (
                <div className="col-span-full rounded-[24px] bg-white p-12 text-center text-slate-500 shadow-sm">
                  No products match this vehicle selection yet. Try changing the bike details from the finder.
                </div>
              ) : (
      ...
      <Drawer open={mobileMenuOpen} onClose={() => setMobileMenuOpen(false)} title="Menu & Filters">
        <MobileMenuFilters
          ...
          selectedBikeBrand={selection.make || "all"}
          setSelectedBikeBrand={() => {}} // No brand change from finder results mobile menu for now
