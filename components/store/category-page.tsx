...
export default function CategoryPage({
  ...
  const [activeFilter, setActiveFilter] = useState<any>(category);
        ...
        <section className="bg-slate-950 py-18">
          <div className="mx-auto max-w-[1400px] px-4 py-16 text-white lg:px-6">
            ...
            <h1 className="font-heading text-4xl font-extrabold sm:text-5xl">
              {meta?.title ?? "Motorbike Parts"}
            </h1>
            <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg sm:leading-8">
              {meta?.copy ?? "Browse all products in this motorbike parts category."}
            </p>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
            <div className="mb-8 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
              <p className="text-sm font-medium text-slate-500">
                Showing {filtered.length} products in {meta?.title ?? category}
              </p>
            ...
            <div className="mb-8 grid gap-3 rounded-[28px] bg-white p-4 shadow-sm md:grid-cols-3">
              ...
              <label className="relative">
                <select
                  ...
                  onChange={(event) =>
                    setSelectedBikeBrand(event.target.value)
                  }
